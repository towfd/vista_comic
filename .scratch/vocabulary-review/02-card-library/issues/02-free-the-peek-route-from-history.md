...
- [ ] The peek route and its supporting types live outside `Features/History/`
- [ ] 歷史紀錄 still uses it, unchanged, and still behaves identically
- [ ] Jumping to a page still does **not** write reading progress
- [ ] A source comic that is no longer in the library still degrades the same way
- [ ] Existing tests covering the route move with it and still pass, without being rewritten to fit
- [ ] No behaviour change is introduced anywhere; the diff is a move plus whatever the move requires
