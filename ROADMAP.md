...
- [x] **Slice 4 — reading-position persistence** (`backend-implementer` then `frontend-implementer`, sequential). Shipped across PRs #14–#17 (2026-07-23 – 2026-07-24).
  ...
  - Pull-past-bottom auto-advance restarts the next chapter at page 1 and overwrites its saved progress; explicit jumps still resume. **Later corrected** ([#65](https://github.com/towfd/vista_comic/pull/65), 2026-08-10): it inferred the pull from scroll geometry alone, which on iPad a keyboard-driven content collapse also satisfied — see `.scratch/reader-auto-advance-false-trigger/`. The "verified on iPhone" below was true and still missed it; the sheet covers the reader on iPhone, so the keyboard never resizes the presenter.
...
### Offline download (post-M10, ticket-driven)

- Spec of record: `.scratch/offline-download/spec.md`

Download chapters deliberately and read them with no connection at all. Eight tickets: the download engine and its store, offline reading, the catch-up queue for progress, the cap and its eviction, the 已下載 screen, batch download, and two that the repo owner's device passes added (comic covers, and offline read state).

**The download is deliberate, and that single rule is what holds the feature together.** Pages that merely passed through the Reader are never kept. It keeps three things equal that would otherwise drift apart — what 已下載 lists, what counts against the cap, and what is actually on the device — and it is why the image cache reads from disk but never writes to it. The one exception is a separate store for comic covers, which counts against nothing.

**The metadata was the hard part, not the bytes.** A chapter's ordered page URLs are only ever known from a live reader request, so twenty chapters of images on the device would still be unreachable. Each download therefore carries its own record — ids, titles, page URLs, page count, started-at, completion — written before the first page is fetched. The 已下載 screen needs no catalog for the same reason: it names comics from those records, which is the only way a list can work in the situation it exists for.

**Offline reading is one step in front of the network, in one place.** Every image the app loads reaches the network through a single closure inside the cache, so a single disk-first step there is the whole feature and the Reader gains no offline code path at all. The catalog side is a decorator over the live repository: `library()` and `comic(id:)` replay the raw bytes of the last successful response — bytes, because the display models are `Decodable` only — while the reader endpoint answers from a **completed** chapter record instead, since a stored response would hand back a page list for pages that are not here.

**Only a transport failure counts as offline.** A 500, an auth rejection at the edge, or an undecodable response are re-thrown. Serving yesterday's library after a real server error looks perfectly fine on screen while being silently out of date, and nothing would ever tell the reader.

**The cap evicts rather than refuses**, strictly oldest-first by started-at, one eviction per chapter admitted — so a batch of five evicts exactly five, and there is no batch admission path that could dodge it. Read state is deliberately not consulted: a rule the reader can predict without knowing what the app thinks they have read is worth more than a cleverer one. The chapter open in the Reader is passed over whatever its age, which is the one piece of knowledge the Reader had to volunteer.

**Progress is queued rather than discarded.** Swallowing a failed progress write was right when a failure meant a blip; once reading offline is normal, the same rule loses a flight. One entry per chapter, last write wins, persisted, and flushed *before* the three requests that display progress — so landing shows where you got to rather than yesterday, without a second refresh.

**Two accepted failure modes, recorded so they are not later mistaken for defects.** A chapter saved for a journey can be evicted by later downloads before it is read; 已下載 makes that queue visible so it can be seen coming. And chapter thumbnails are not kept offline — one per chapter is 3,000+ images at this library's size, against one per comic for covers — so the chapter list shows placeholder artwork with no connection.

## Known issues and constraints

- **`PageImageCacheTests` has three intermittently failing tests**, and the attempt to fix them was reverted ([#75](https://github.com/towfd/vista_comic/pull/75), closed). The captured assertions are `(peakConcurrentLoads → 3) == 4`, `(order[4] → …/priority/4) == urls[5]` and `(requests.count → 7) == 6`, and one explanation fits all three: the prefetch window is fire-and-forget, so a test can return while the cache is still fetching and those requests are recorded against whichever test runs next — and nearly every assertion in that suite is an exact count. Two of the three additionally assert a scheduling accident rather than a guarantee. The rewrite that addressed this hung the serialized suite instead of fixing it, so it was withdrawn; the analysis is on the closed branch. Whoever picks it up should capture failures with `-resultBundlePath` pointed somewhere writable, since the default location fails here and leaves nothing but a test name.
...
**Offline download is merged in full** ([#68](https://github.com/towfd/vista_comic/pull/68), [#69](https://github.com/towfd/vista_comic/pull/69), [#70](https://github.com/towfd/vista_comic/pull/70), [#71](https://github.com/towfd/vista_comic/pull/71), [#72](https://github.com/towfd/vista_comic/pull/72), [#73](https://github.com/towfd/vista_comic/pull/73), [#74](https://github.com/towfd/vista_comic/pull/74), [#76](https://github.com/towfd/vista_comic/pull/76)), each ticket device-verified by the repo owner before the next was started. Two of the eight tickets did not exist when the spec was written: comic covers and offline read state were both found on a device and specified afterwards.

**Otherwise nothing is in progress. The next action is unclaimed**, in rough order of how much each is already owed:

- **The three flaky prefetch tests**, now that the failures are captured (see Known issues). Worth doing as its own effort, run by the repo owner rather than in this environment.
