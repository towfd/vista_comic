...
**In flight:** [#65](https://github.com/towfd/vista_comic/pull/65) — three reader changes found by reading on an iPad, awaiting merge. Auto-advance no longer fires on a content collapse; pages reserve their real height while their image is not in memory; selection mode ends itself once a selection is made. Trackers: `.scratch/reader-auto-advance-false-trigger/`, `.scratch/reader-page-prefetch/issues/03` (reopened and resolved), `.scratch/reader-selection-mode-exit/`.

One thing that PR does **not** close: some content still moves when the keyboard appears over the reader. Accepted by the repo owner as much improved, recorded as an undiagnosed residual on `reader-page-prefetch` ticket 03. Measure where it is concentrated before rebuilding anything.
