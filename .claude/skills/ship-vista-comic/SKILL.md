...
description: Ship reviewed vista_comic changes as a pull request. Use when the user has personally reviewed the change and asks to push, ship, publish, or open a PR. Creates a feature branch, verifies the build, commits with project conventions, pushes, and opens a PR. Never pushes to the default branch unless the user explicitly overrides.
---
...
5. End every commit message with the Claude Code co-author trailer, using the model identifier the runtime specifies, for example:
